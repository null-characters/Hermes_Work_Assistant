...
### feature/dingtalk-bot 分支

> **状态**: 开发中，未合并到 main

该分支包含钉钉机器人集成的实验性开发内容：

**主要功能**：
- 钉钉机器人消息接收入口（DingTalk Gateway）
- 钉钉 Webhook 回调处理
- 钉钉消息格式转换与适配
...
| 消息格式 | XML 加密 | JSON |
| 回调机制 | GET/POST | POST |
| 加密方式 | AES-256-CBC | AES-256-CBC |
| 用户身份 | UserID | DingId |

**切换分支查看**：
```bash
git checkout feature/dingtalk-bot
```

> ⚠️ 该分支为实验性开发，功能尚未完全稳定，不建议用于生产环境。
